...
description: >
  Drupal migration expertise. Use when working with D7-to-D10 migrations, CSV imports, JSON API imports, or custom
  migration plugins.
metadata:
  status: stable
  last-reviewed: 2026-07
  reference: https://github.com/madsnorgaard/drupal-agent-resources
---

# Drupal Migration Expert

You are an expert in Drupal's Migrate API, helping with D7 to D10/D11 migrations, CSV imports, and custom data
migrations.
...
ddev drush en migrate

# Contrib essentials
ddev composer require drupal/migrate_plus drupal/migrate_tools drupal/migrate_file drupal/migrate_source_csv
ddev drush en migrate_plus migrate_tools migrate_file migrate_source_csv
```
...
Migration groups are configurations that group related migrations together, providing shared metadata and organization.

### Group YAML Structure

```yaml
# config/install/migrate_plus.migration_group.my_group.yml
id: my_group
label: 'My Group'
description: 'Group for related migrations'
source_type: 'MySQL'  # Optional: describes source system
```

### Register Group via Hooks

```php
/**
 * Implements hook_migration_group_alter().
 */
function my_module_migration_group_alter(array &$groups) {
  // Override or extend group properties
}
```

### Using Groups in Migrations

Reference the group in each migration YAML:

```yaml
id: my_migration
label: 'My Migration'
migration_group: my_group  # Must match the group id
...
### Drush Commands with Groups

```bash
# List only migrations in a group
drush migrate:status --group=my_group
...
| Group ID | Use Case |
|----------|----------|
| `migrate_drupal_7` | Auto-generated D7 upgrade migrations |
| `imports` | CSV or custom data imports |
| `my_feature` | Feature-specific migrations |
